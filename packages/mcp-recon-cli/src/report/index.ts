...
export function renderMarkdown(input: RenderInput): string {
  ...
  const sortedClassifications = [...classification.classifications].sort((a, b) => {
    ...
    if (ord !== 0) return ord;
    if (a.data_class !== b.data_class) return a.data_class.localeCompare(b.data_class);
...
function renderTool(c: Classification, description: string, fuzzStats?: ReturnType<typeof toolFuzzSummary>): string[] {
  const out: string[] = [];
