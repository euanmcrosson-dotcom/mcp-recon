...
export const PRIVILEGED_VERBS = [
	/\b(spawn|exec|shell|bash|cmd|invoke_program|fork)\b/i,
];
...
export const RULES: readonly NameOrDescRule[] = [
	...
	{
		pattern:
			/\b(read[_-]?(file|text[_-]?file|media[_-]?file|multiple[_-]?files))\b/i,
	...
	{
		pattern:
			/\b(list[_-]?(directory|directory[_-]?with[_-]?sizes)|directory[_-]?tree)\b/i,
		data_class: "filesystem",
		authority_floor: "read",
		scope: "name",
	},
	{
		pattern:
			/\b(get[_-]?file[_-]?info|search[_-]?files|list[_-]?allowed[_-]?directories)\b/i,
		data_class: "filesystem",
		authority_floor: "read",
		scope: "name",
	},
	{
		pattern:
			/\b(write[_-]?file|edit[_-]?file|create[_-]?directory|move[_-]?file)\b/i,
		data_class: "filesystem",
	...
	{
		pattern:
			/\b(send[_-]?(email|message|sms|notification)|notify|publish|broadcast)\b/i,
	...
	{
		pattern:
			/\b(get[_-]?env|environ(ment)?|process[_-]?(info|status)|sysinfo|system[_-]?info|hostname)\b/i,
		data_class: "system",
		authority_floor: "read",
		scope: "either",
	},
	...
	{
		pattern:
			/\b(echo|get[_-]?(structured[_-]?content|annotated[_-]?message|resource[_-]?(links|reference)|sum|tiny[_-]?image))\b/i,
		data_class: "metadata",
		authority_floor: "read",
		scope: "name",
	},
	...
	{
		pattern:
			/\b(trigger[_-]?(long[_-]?running[_-]?operation)?|simulate[_-]?(research[_-]?query))\b/i,
		data_class: "metadata",
		authority_floor: "write",
		scope: "name",
	},
