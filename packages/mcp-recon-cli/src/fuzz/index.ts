...
export async function fuzz(
  ...
  const perTool: Record<string, { ok: number; protocol_error: number; runtime_error: number }> =
    {};
...
function* generateAllAxes(
  ...
  prng: Prng,
): Generator<{ axis: FuzzAxis; args: Record<string, unknown>; rationale: string }> {
  for (const v of generateBoundary(facts, prng)) yield { axis: "boundary_values", ...v };
  for (const v of generateTypeConfusion(facts, prng)) yield { axis: "type_confusion", ...v };
  for (const v of generateEncoding(facts, prng)) yield { axis: "encoding_tricks", ...v };
  for (const v of generatePathTraversal(facts, prng)) yield { axis: "path_traversal", ...v };
  for (const v of generateUrlHostility(facts, prng)) yield { axis: "url_hostility", ...v };
  for (const v of generateSchemaViolation(facts, prng)) yield { axis: "schema_violation", ...v };
}
