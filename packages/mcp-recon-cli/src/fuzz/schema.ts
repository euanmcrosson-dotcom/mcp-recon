...
export function extractToolFacts(name: string, inputSchema: unknown): ToolFacts {
  ...
  for (const [argName, argSchema] of Object.entries(properties)) {
    ...
    const enumValues = Array.isArray(enumRaw) ? (enumRaw as readonly unknown[]) : undefined;
