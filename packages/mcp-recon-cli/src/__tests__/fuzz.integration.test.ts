...
describe("fuzz against @modelcontextprotocol/server-filesystem (LIVE_MCP)", () => {
  ...
  it("most calls land on protocol_error — server validates inputs", async () => {
    ...
    const totalProto = result.summary.reduce((acc, s) => acc + s.protocol_error, 0);
    const totalRuntime = result.summary.reduce((acc, s) => acc + s.runtime_error, 0);
  ...
  it("re-running with the same seed produces the same call shape", async () => {
    const inv = await enumerate(client);
    const a = await fuzz(client, inv, { budget: 4, seed: 42, timeoutMs: 10_000 });
    const b = await fuzz(client, inv, { budget: 4, seed: 42, timeoutMs: 10_000 });
    ...
    expect(a.calls.map((c) => c.rationale)).toEqual(b.calls.map((c) => c.rationale));
  }, 180_000);
