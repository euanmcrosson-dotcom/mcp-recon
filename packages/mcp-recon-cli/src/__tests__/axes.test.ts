...
const ARGLESS_TOOL = extractToolFacts("noargs", { type: "object", properties: {} });
...
describe("generateBoundary", () => {
  ...
  it("covers boundary values for string args", () => {
    ...
    expect(values.some((v) => typeof v === "string" && (v as string).length > 1000)).toBe(true);
  });
...
describe("generateEncoding", () => {
  ...
  it("includes percent-encoded NUL + Cyrillic homograph + RTL override", () => {
    ...
    expect(values.some((v) => typeof v === "string" && (v as string).includes("‮"))).toBe(true);
  });
});
...
describe("generatePathTraversal", () => {
  ...
  it("includes classic POSIX + Windows + percent-encoded shapes", () => {
    ...
    expect(values.some((v) => typeof v === "string" && (v as string).includes("%2e"))).toBe(true);
