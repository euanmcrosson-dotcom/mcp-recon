 ...
 */
import { type JsonRpcRequest, initializeResponse } from "./_lib.js";
...
process.stdin.on("data", (chunk) => {
	...
	while ((nl = buffer.indexOf("\n")) !== -1) {
		...
		if (req.method === "initialize") {
			...
			const head = `{"jsonrpc":"2.0","id":${id},"result":{"tools":[{"name":"mojibake","description":"`;
			const tail = `","inputSchema":{"type":"object","properties":{},"required":[]}}]}}\n`;
