...
describe("planCaveats — top-level shape", () => {
	...
	it("emits one plan per classification entry", () => {
		const out = planCaveats(SAMPLE_CLASSIFICATION, FULL_BINDINGS);
		expect(out.plans).toHaveLength(
			SAMPLE_CLASSIFICATION.classifications.length,
		);
	});
...
describe("planCaveats — placeholder substitution", () => {
	...
	it("substitutes <your-sandbox-prefix> with bindings.sandbox_prefix", () => {
		...
		expect(
			write.caveats.some((c) => c.includes("/var/agent-sandbox/tenant-42")),
		).toBe(true);
		expect(
			write.caveats.every((c) => !c.includes("<your-sandbox-prefix>")),
		).toBe(true);
	});
	...
	it("substitutes <your-cap-expiry> with bindings.expiry", () => {
		const out = planCaveats(SAMPLE_CLASSIFICATION, FULL_BINDINGS);
		for (const plan of out.plans) {
			expect(plan.caveats.every((c) => !c.includes("<your-cap-expiry>"))).toBe(
				true,
			);
		}
		const read_file = out.plans.find((p) => p.tool === "read_file")!;
		expect(
			read_file.caveats.some((c) => c.includes("2026-05-01T18:00:00.000Z")),
		).toBe(true);
	});
	...
	it("multi-arg path substitution: move_file gets both source and destination bound", () => {
		...
		expect(
			move.caveats.some((c) =>
				c.includes('arg.source starts_with "/var/agent-sandbox/tenant-42/"'),
			),
		).toBe(true);
		expect(
			move.caveats.some((c) =>
				c.includes(
					'arg.destination starts_with "/var/agent-sandbox/tenant-42/"',
				),
			),
		).toBe(true);
	});
	...
	it("leaves placeholder literal when binding is omitted", () => {
		...
		const write = out.plans.find((p) => p.tool === "write_file")!;
		expect(write.caveats.some((c) => c.includes("<your-sandbox-prefix>"))).toBe(
			true,
		);
	});
});
...
describe("planCaveats — per_tool_overrides", () => {
	it("appends override caveats after substituted ones", () => {
		...
		const write = out.plans.find((p) => p.tool === "write_file")!;
		expect(write.caveats[write.caveats.length - 1]).toBe(
			'arg.path matches "\\\\.txt$"',
		);
	});
