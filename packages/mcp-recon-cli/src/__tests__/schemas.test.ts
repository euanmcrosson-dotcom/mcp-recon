...
const EXAMPLES = {
  ...
  classification: "examples/public-servers/server-filesystem/classification.json",
} as const;

describe("schemas/*.v0.1.json — schema files are themselves valid JSON Schemas", () => {
  it.each(Object.entries(SCHEMAS))("compiles %s schema with Ajv 2020-12", (_name, path) => {
...
describe("schemas/classification.v0.1.json", () => {
  ...
  it("rejects a missing required field (recommended_caveat)", () => {
    const broken = JSON.parse(JSON.stringify(example));
    delete broken.classifications[0].recommended_caveat;
    expect(validate(broken)).toBe(false);
  });
});
  ...
  const example = {
    ...
    plans: [
      ...
      },
      {
        ...
        caveats: ['tool == "write_file"', 'arg.path starts_with "/tmp/sandbox/"'],
        flagged: true,
