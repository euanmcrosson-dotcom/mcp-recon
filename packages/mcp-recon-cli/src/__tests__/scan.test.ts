...
describe("scan orchestrator", () => {
  ...
  it("emits caveats.json when only --caller is provided", async () => {
    ...
    const cav = JSON.parse(fs.readFileSync(path.join(outDir, "caveats.json"), "utf8"));
  ...
  it("emits caveats.json with full bindings matching what planCaveats produces directly", async () => {
    ...
    const fuzzResults = await fuzz(refClient, inventory, { budget: 3, timeoutMs: 1000, seed: 42 });
    ...
    const onDisk = JSON.parse(fs.readFileSync(path.join(outDir, "caveats.json"), "utf8"));
