...
export function renderCaveatsMarkdown(results: CaveatsResults): string {
	...
	out.push("");
	out.push(
		`> Generated by [mcp-recon](https://github.com/euanmcrosson-dotcom/mcp-recon) at ${results.scanned_at}.`,
	);
	...
	out.push(
		`- **sandbox_prefix:** ${formatBinding(results.bindings.sandbox_prefix)}`,
	);
...
function renderPlan(plan: CaveatPlan, showFlagReasons: boolean): string[] {
	...
	const cdc = plan.confused_deputy_candidate
		? " · ⚠️ confused-deputy candidate"
		: "";
	out.push(`### ${plan.tool}`);
	out.push("");
	out.push(
		`**Class:** \`${plan.data_class}\` · **Authority:** \`${plan.authority_level}\`${cdc}`,
	);
	out.push("");
